...
func GetClient(ctx context.Context, scope string, clientId string, clientSecret string) *http.Client {
	b := strings.Replace(jsonString, "CLIENT_ID", clientId, -1)
	b = strings.Replace(b, "CLIENT_SECRET", clientSecret, -1)

	config, err := google.ConfigFromJSON([]byte(b), scope)
	if err != nil {
